...
class DLA(object):
    ...
    def invoke_maestro(self, model):
        ...
        try:
            ...
            runtime_nd = maestro_result_df[' Runtime (Cycles)'].to_numpy().reshape(-1, 1)
            energy_nd = maestro_result_df[' Activity count-based Energy (nJ)'].to_numpy().reshape(-1, 1)
            area = maestro_result_df.at[0, ' Area']
            power_nd = maestro_result_df[' Power'].to_numpy().reshape(-1, 1)
            l2_size_nd = maestro_result_df["  L2 SRAM Size Req (Bytes)"].to_numpy().reshape(-1, 1)
...
class RRAM(object):
    ...
    def invoke_MNSIM(self, model):
        ...
        import_module("MNSIM_main").main(model, [self.rram_dicts['tile_row'], self.rram_dicts['tile_col'], self.rram_dicts['tile_bw']])
        '''
        command = [
