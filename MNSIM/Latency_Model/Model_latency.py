...
class Model_latency():
    ...
    def pipe_result_update(self,
        ...
        if layer_type == 'conv':
            ...
            self.buffer_latency[layer_id].append(temp_tile_latency.tile_buf_wlatency + temp_tile_latency.tile_buf_rlatency + temp_tile_latency.PE_buf_rlatency +
                                                 temp_tile_latency.PE_buf_wlatency)
            ...
            self.digital_latency[layer_id].append(temp_tile_latency.iReg_latency + temp_tile_latency.input_demux_latency +
                                                  temp_tile_latency.output_mux_latency + temp_tile_latency.shiftreg_latency + temp_tile_latency.adder_latency +
                                                  temp_tile_latency.oReg_latency + temp_tile_latency.jointmodule_latency)
    ...
    def calculate_model_latency(self, mode=0):
        ...
        for layer_id in range(len(self.NetStruct)):
            ...
            if layer_id == 0:
                ...
                temp_tile_latency.outbuf.calculate_buf_read_latency(rdata=(self.graph.layer_tileinfo[layer_id]['max_column'] * outputbit *
                                                                           self.graph.layer_tileinfo[layer_id]['max_PE'] / 8))
                                ...
                                if (i == 0) & (j == 0):
                                    ...
                                    if mode == 0:
                                        if cur_multiple == 1:
                                            ...
                                            temp_insize = inoutsize_conversion(kernelsize=kernelsize,
                                                                               padding=padding / 2,
                                                                               stride=stride,
                                                                               outputsize=split_size[m])  # only one padding column
                                            indata = input_channel_PE * (temp_insize * max(kernelsize - padding - 1, 0) +
