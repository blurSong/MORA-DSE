    ...
    def __init__(self, hardware_config, layer_config, quantize_config):
        ...
        if self.layer_config['type'] == 'conv':
            ...
            self.layer_list = nn.ModuleList([
                nn.Conv2d(i,
                          self.layer_config['out_channels'],
                          self.layer_config['kernel_size'],
                          stride=self.layer_config['stride'],
                          padding=self.layer_config['padding'],
                          dilation=1,
                          groups=1,
                          bias=False) for i in in_channels_list
            ])
        ...
        self.register_buffer(
            'bit_scale_list',
            torch.FloatTensor([[quantize_config['activation_bit'], -1], [quantize_config['weight_bit'], -1], [quantize_config['activation_bit'], -1]]))
